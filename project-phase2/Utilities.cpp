...
std::string Utilities::prompt(std::string output, std::string reg ,std::list<std::string> acceptions){
...
std::list<Account> Utilities::loadAccountInformation(std::string accounts_file){
...
Account *Utilities::getAccountFromName(std::string name, std::list<Account> accounts){
	bool found = false;
...
Account *Utilities::getAccountFromNumber(double number, std::list<Account> accounts){
	bool found = false;
	std::list<Account>::iterator it = accounts.begin();
	for(int i=0; i<accounts.size(); i++){
